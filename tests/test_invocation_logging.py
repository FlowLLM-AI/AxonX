...
async def test_run_job_logs_name_and_bounded_arguments():
    ...
    assert len(messages[0].removeprefix("Job called: name=inspect arguments=")) <= LOG_ARGUMENT_MAX_LENGTH
...
async def test_plugin_endpoint_logs_name_and_upload_metadata():
    ...
    assert len(messages) == 1
    assert messages[0].startswith("Plugin endpoint called: name=install_plugin arguments=")
