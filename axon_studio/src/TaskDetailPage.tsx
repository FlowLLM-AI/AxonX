  ...
  return <section className="workspace-page task-detail-page">
    ...
    <div className="task-detail-layout">
      <div className="task-detail-summary">
        ...
        <DetailSection title={text.steps}><div className="step-list">{task.steps.length ? task.steps.map((step, index) => <div className="step-item" key={`${step.name}-${index}`}><span className={step.finished_at ? "done" : "active"}>{step.finished_at ? "✓" : index + 1}</span><div><strong>{step.name}</strong><small>{step.finished_at ? formatDate(step.finished_at, language) : step.started_at ? `${Math.round(step.percentage || 0)}%` : text.notStarted}</small></div></div>) : <p className="muted">{text.notStarted}</p>}</div></DetailSection>
