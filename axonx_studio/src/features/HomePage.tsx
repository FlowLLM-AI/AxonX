...
const quantSteps: { id: SectionId; zh: string; en: string; detailZh: string; detailEn: string }[] = [
  ...
  return (
    <section className="workspace-page overview-page">
      <div className="overview-hero">
        <section className="hero-quant" aria-label={zh ? "量化基础框架" : "Quant research framework"}>
          <div className="hero-quant-heading">
            <span>01 / QUANT RESEARCH</span>
            <h2>{zh ? "量化基础框架" : "Quant research"}</h2>
            <p>{zh ? "从数据准备到策略验证" : "From market data to strategy validation"}</p>
          </div>
          <ol className="hero-quant-steps">
            {quantSteps.map((step, index) => (
              <li key={step.id}>
                <button className={`hero-quant-step hero-quant-${step.id}`} onClick={() => onNavigate(step.id)}>
                  <span className="hero-quant-icon"><QuantIcon id={step.id} /></span>
                ...
                {index < quantSteps.length - 1 && <ArrowDown className="hero-quant-connector" aria-hidden="true" />}
              </li>
            ))}
          </ol>
        </section>

        <section className="hero-brand" aria-label={zh ? "AxonX 量化 Harness 框架" : "AxonX Quant Harness"}>
          <div className="hero-brand-halo" aria-hidden="true"><span /><span /><span /></div>
        ...
        <section className="hero-harness" aria-label={zh ? "Harness 框架能力" : "Harness framework capabilities"}>
          <div className="hero-harness-heading">
            <span>02 / HARNESS CORE</span>
            <h2>{zh ? "Harness 框架能力" : "Harness capabilities"}</h2>
            <p>{zh ? "从插件扩展到产物追踪" : "From plugins to artifact lineage"}</p>
          </div>
            ...
            {harnessFeatures.map((feature) => {
              ...
              return (
                <div className="hero-harness-feature" key={feature.code}>
                  <span className="hero-harness-icon"><Icon aria-hidden="true" /></span>
                  <div>
