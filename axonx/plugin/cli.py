"""Local CLI for inspecting and managing installed AxonX plugin packages."""
...
from .manifest import PluginManifest, load_package_manifest, parse_plugin_manifest


@dataclass(frozen=True)
class InstalledPlugin:
    """Package metadata for one installed ``axonx.plugins`` entry point."""
...
def _list_plugins(args: argparse.Namespace) -> int:
    ...
    if args.json:
        ...
        return 0
    if not records:
        print("No AxonX plugins installed.")
        return 0
    headers = ["PLUGIN", "DISTRIBUTION", "VERSION"]
...
def _installed_manifest(plugin: InstalledPlugin) -> PluginManifest:
    if ":" in plugin.target:
        raise ValueError(
            f"Plugin entry point '{plugin.name}' must target a package containing plugin.yaml, got '{plugin.target}'",
        )
    ...
    # may contain registration side effects.
...
def _plugin_details(plugin: InstalledPlugin) -> dict:
    ...
    jobs = manifest.config.get("jobs") or {}
...
def _install_plugin(args: argparse.Namespace) -> int:
    ...
    if result == 0:
        print("Package installed. Run 'axonx plugin list' to verify it, then enable its plugin name in app config.")
    return result
...
def _validate_installed(name: str) -> list[str]:
    from .runtime import PluginManager
...
def _validate_local(path: Path) -> list[str]:
    from ..components.component_registry import R
    from .runtime import PluginManager, _plugin_from_manifest
...
def _validate_plugin(args: argparse.Namespace) -> int:
    ...
    print(f"Valid AxonX plugin: {', '.join(names)}")
    return 0


def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="axonx plugin", description="Manage AxonX plugin packages.")
    commands = parser.add_subparsers(dest="command", required=True)

    list_parser = commands.add_parser("list", help="List installed AxonX plugins.")
...
def plugin_cli(argv: Sequence[str]) -> int:
    """Run the local plugin command group and return its exit status."""
