"""Search task artifacts and build their ETL lineage graph."""
...
from .browser import _workspace_root
...
def _list_task_graphs(root: Path, query: str, offset: int, limit: int) -> dict[str, Any]:
...
def _get_task_graph(root: Path, task_id: str) -> dict[str, Any]:
    nodes = _task_index(root)
    ...
    graph = {
        ...
        if _root_id(nodes, node_id) == root_id
    }
    ...
    return {
        "root_id": root_id,
        "selected_id": task_id,
        "nodes": sorted(graph.values(), key=lambda node: (KINDS.index(node["kind"]), node["created_at"] or "", node["task_id"])),
        "edges": [
            {"from": node["parent_id"], "to": node["task_id"]}
            for node in graph.values() if node["parent_id"]
        ],
    }
...
class ListTaskGraphsStep(BaseStep):
    async def execute(self):
        self.response.answer = await asyncio.to_thread(
            _list_task_graphs, _workspace_root(self),
            self.context.get("q", ""), self.context.get("offset", 0), self.context.get("limit", 50),
        )
...
class GetTaskGraphStep(BaseStep):
    async def execute(self):
        self.response.answer = await asyncio.to_thread(
            _get_task_graph, _workspace_root(self), self.context["task_id"],
        )
