...
class RankingBacktestTask(BaseTask):
    ...
    def normalize_boolean(values: pd.Series, column: str) -> pd.Series:
        ...
        for value in values:
            ...
            elif isinstance(value, (int, float, np.integer, np.floating)) and value in (
                0,
                1,
            ):
                normalized.append(bool(value))
        ...
        try:
            with PdfPages(temporary) as pdf:
                ...
                for top_n in self.context["top_ns"]:
                    axis.plot(
                        dates,
                        daily[f"top{top_n}_gross_return"].cumsum(),
                        label=f"Top {top_n}",
                        linewidth=1,
                    )
                axis.plot(
                ...
                for top_n in self.context["top_ns"]:
                    axis.plot(
                        dates,
                        daily[f"top{top_n}_net_value"] - 1.0,
                        label=f"Top {top_n}",
                        linewidth=1,
                    )
                axis.set_title(
    ...
    def draw_quarterly_performance(
        ...
        axis.bar(
            ...
            summary["return"],
            width=width,
            label="Arithmetic cumulative return",
        )
        axis.bar(
            positions + width / 2,
