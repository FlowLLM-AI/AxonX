...
class LgbmPredictionTask(BaseTask):
    ...
    def resolve_training_task(self) -> None:
        training_dir = task_directory(
            self.workspace_path, "training", self.config.training_task_id
        )
        training_metadata_path = training_dir / "metadata.json"
        training_metadata = read_metadata(
            training_metadata_path, description="LightGBM training"
        )
        ...
        expected_hash = (
            training_metadata.get("artifact_integrity", {})
            .get("model", {})
            .get("sha256")
        )
    ...
    def resolve_source_dataset(self) -> None:
        etl_task_id = (
            self.context["training_metadata"].get("source", {}).get("etl_task_id")
        )
        ...
        self.logger.info(
            f"Prediction dataset resolved etl_task_id={etl_task_id} dataset={dataset_path}"
        )
    ...
    def load_and_validate_prediction_data(self) -> None:
        ...
        if pred_end > latest:
            self.logger.warning(
                f"pred_end={pred_end} 晚于 ETL 最新日期 {latest}; 将截断到最新日期"
            )
            pred_end = latest
        frame = (
            pl.scan_parquet(path)
            .filter(
                (pl.col("trade_date") >= pl.lit(self.config.pred_start))
    ...
    def write_outputs(self) -> None:
        ...
        atomic_output(
            path,
            lambda temporary: self.context["predictions"].write_parquet(
    ...
    def write_metadata(self) -> None:
        ...
        metadata = {
            **metadata_header(
            ...
            "model_target": self.context["training_metadata"]["protocol"][
                "label_column"
            ],
