...
class TushareDownloadInputParams(BaseInputParams):
    ...
    days_back: int = 7
    timeout: int = TUSHARE_TIMEOUT_SECONDS
    ...
    def normalize_datasets(cls, value: Any) -> str:
        ...
        if not isinstance(value, str):
            raise ValueError("datasets 必须是逗号分隔字符串或字符串列表")  # noqa: TRY004
        selected = tuple(
...
@provider("download_tushare_task")
class DownloadTushareTask(BaseTask):
    ...
    def initialize(self) -> None:
        ...
        end = min(self._parse_date(self.input_params.end_date) or today, today)
        ...
        months = tuple(
            self._month_bounds(day) for day in days if day.day == 1 or day == start
        )
        self.state.update(
    ...
    def download_hs300_weights(self) -> None:
        ...
        self.logger.info(
            f"HS300 weight download completed rows={self.state['rows']['index_weight']}"
        )
    ...
    def sort_output_files(self) -> None:
        ...
        self.state["files"].sort(
            key=lambda value: (
                ...
                dataset_order.get(
                    os.path.splitext(os.path.basename(value))[0], len(dataset_order)
                ),
                value,
            ),
        )
    ...
    def _download_days(self, api_name: str) -> None:
        ...
        self.logger.info(
            f"Dataset download completed dataset={api_name} rows={self.state['rows'][api_name]}"
        )

    def _query(
        self, api_name: str, *, paginated: bool = False, **params
    ) -> pd.DataFrame:
        fields = ",".join(DATASETS[api_name])
        self.logger.info(f"Tushare API query api={api_name} params={params}")
        query = (
            self.state["client"].query_has_more
            if paginated
            else self.state["client"].query
        )
    ...
    def _save_static(self, api_name: str, frame: pd.DataFrame) -> None:
        ...
        path = self.state["root"] / f"{api_name}.parquet"
    ...
    def _save(self, api_name: str, frame: pd.DataFrame, day: date) -> None:
        ...
        frame = frame.loc[:, DATASETS[api_name]].sort_values(
            sort_columns, kind="stable", ignore_index=True
        )
        path = (
            self.state["root"] / f"{day:%Y}" / f"{day:%Y%m%d}" / f"{api_name}.parquet"
        )
        self._write_frame(api_name, frame, path)
    ...
    def _write_frame(self, api_name: str, frame: pd.DataFrame, path: Path) -> None:
        ...
        self.state["files"].append(str(path))
        self.state["rows"][api_name] += len(frame)
        self.logger.info(
    ...
    def _parse_date(value: Any) -> date | None:
        ...
        if text.isdigit():
            if len(text) != 8:
                raise ValueError(f"Invalid compact date: {text!r}")
