...
class DownloadTushareTask(BaseTask):
    ...
    def initialize(self) -> None:
        ...
        start = self._parse_date(self.config.start_date) or end - timedelta(
            days=self.config.days_back - 1
        )
        if start > end:
    ...
    def _save(self, api_name: str, frame: pd.DataFrame, day: date) -> None:
        ...
        try:
            frame.to_parquet(
                temporary, index=False, compression="zstd", compression_level=6
            )
    ...
    def _parse_date(value: Any) -> date | None:
        ...
        return datetime.strptime(
            text, "%Y%m%d" if text.isdigit() else "%Y-%m-%d"
        ).date()
