...
from ..storage.metadata import write_task_metadata
...
class TaskRunner:
    ...
    def _publish(self) -> None:
        task = self.task
        status = self.status
        assert task is not None and status is not None
        # Journal progress before publishing the matching status snapshot. A
